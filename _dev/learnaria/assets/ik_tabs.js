;(function ( $, window, document, undefined ) {
	...
	Plugin.prototype.init = function () {
		...
		$tabbar = $('<ul/>') // create ul element to hold all tabs
			...
			.attr({'role': 'tablist'}) // add tablistr role (Criteria 1 and Criteria 2)
			.prependTo($elem);
		...
		plugin.panels = $elem // initialize panels and create tabs
			...
			.each( function(i, el) {
				...
				$panel = $(el).attr({
					...
					'role': 'tabpanel', // add tabpanel role (Criteria 1)
				...
				$tab = $('<li/>').attr({
					...
					'role': 'tab', // assign tab role (Criteria 1)
	...
	Plugin.prototype.selectTab = function (event) {
		...
		$tabs // deselect all tabs
			...
			.attr({
				'aria-selected': false,
				'tabindex': -1 // remove them from tab order	(Criteria 4)
			})
			.blur();

		$($tabs[ind]) // select specified tab (Criteria 4)
			.addClass('selected')
			.attr({
				'aria-selected': true,
				'tabindex': 0
		...
		$($tabs[ind]).focus(); // move focus to current tab if reached by mouse or keyboard (Criteria 3 & 4)
		...
		$($panels[ind]) // show current panel (Criteria 3)
			.attr({'aria-hidden': false})
			.attr({'tabindex': 0})
			.show(); //Criteria 5 & 6 (focus goes from taxindex=0 to next div that is display:block)
