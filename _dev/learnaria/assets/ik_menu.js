;(function ( $, window, document, undefined ) {
	...
	Plugin.prototype.init = function () {
		...
		$('<div/>') // add div element to be used with aria-described attribute of the menu (Criteria 1)
		...
		$elem.find('ul:eq(0)')
			.attr({
				'id': id,
				'role': 'menubar', // assign menubar role to the topmost ul element (Criteria 2)
				'tabindex': 0,
				'aria-labelledby': id + '_instructions' // (Criteria 1)
			});

		$elem.find('li>ul').attr({
			'role': 'menu', //(Criteria 3)
		...
		plugin.menuitems = $elem.find('li') // setup menuitems
			...
			.each(function(i, el) {
				...
				$me.attr({
					'role': 'menuitem', // assign menuitem rols (Criteria 3)
					'tabindex': -1,	// remove from tab order (Criteria 4)
	...
	Plugin.prototype.hideSubmenu = function(event) {
		...
		if ($submenu.length) {
			...
			$submenu.children('li').attr({'tabindex': -1}); // (Criteria 4)
		}
	}
