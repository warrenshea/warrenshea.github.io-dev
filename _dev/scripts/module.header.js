  ...
  return {
    ...
    addEventListeners: () => {
      ...
      document.addEventListener("mousedown", (e) => {
        ...
        if (self.getIsSignInOpen() && storm_eagle.client.viewport.is_xl_up()) {
    ...
    populateNav: (menu,heading,overview,resourceMenu,level,index,newid,columnLength,submenuLength) => {
      ...
      if (menu) {
        ...
        menu.forEach((el,index2) => {
          ...
          if (el.subMenu) {
            temp += `  <button id="${tempnewid}_button" aria-expanded="false" aria-controls="${newid}" data-level="${level}" onclick="storm_eagle.header_navigation_ui.handleClick(event,this,${level},'${tempnewid}');">`;
            temp += `    <span>${el.label}</span>`;
            temp += `  </button>`;
            temp += `  ${storm_eagle.header_navigation_ui.populateNav(el.subMenu,el.label,el.overview,el.resourceMenu,level+1,index2,tempnewid,menu.length,el.subMenu.length)}`;
          } else {
